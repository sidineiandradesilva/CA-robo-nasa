...
public class Posicao {
	...
	public void setOrientacao(char o) {
		this.orient = o;
	}
	...
	public void setPosicao(String p, int tamanho) throws BadRequestException {
		int x_aux = this.getX();
		int y_aux = this.getY();
		for (int i = 0; i < p.length(); i++) {
			switch (p.charAt(i)) {
			case 'M':
				if (this.orient == NORTE)
					x_aux--;
				else if (this.orient == SUL)
					x_aux++;
				else if (this.orient == LESTE)
					y_aux++;
				else if (this.orient == OESTE)
					y_aux--;
				break;
			case 'L':
				if (this.orient == NORTE)
